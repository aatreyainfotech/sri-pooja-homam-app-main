...
export default function CreateAdmin() {
  ...
  const save = async () => {
    ...
    try {
      ...
      Alert.alert('Admin Created', `Admin account created for ${form.full_name}`, [
  ...
  const fields = [
    { key: 'full_name', label: 'FULL NAME', placeholder: 'e.g. Admin Name', icon: 'person' },
    { key: 'mobile', label: 'MOBILE NUMBER', placeholder: '10-digit mobile', icon: 'call', keyboard: 'numeric' as any },
    { key: 'email', label: 'EMAIL (optional)', placeholder: 'admin@example.com', icon: 'mail' },
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 20, gap: 16 }}>
          <View style={styles.infoBox}>
            <Ionicons name="shield-checkmark" size={18} color="#7B1FA2" />
            <Text style={styles.infoText}>
              Admin accounts have access to bookings, poojas, temples, and pujari management. Only Super Admin can create admin accounts.
            </Text>
          </View>
          ...
          <TouchableOpacity onPress={save} style={styles.saveBtn} disabled={saving}>
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveBtnText}>Create Admin Account</Text>
              </>
            )}
          </TouchableOpacity>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  infoBox: {
    flexDirection: 'row', gap: 10, backgroundColor: '#F3E5F5',
    borderRadius: 12, padding: 14, alignItems: 'flex-start',
  },
  infoText: { flex: 1, color: '#7B1FA2', fontSize: 13 },
  ...
  saveBtn: {
    backgroundColor: '#7B1FA2', borderRadius: 14, paddingVertical: 16,
